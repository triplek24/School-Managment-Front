...
const EditUser = () => {
  ...
  const onSubmit = async e => {
    e.preventDefault();
    await axios.post(`https://school-management-app-node.herokuapp.com/student/update/${id}`, user);
    history.push("/home");
  };

  const loadUser = async () => {
    const result = await axios.get(`https://school-management-app-node.herokuapp.com/student/${id}`);
    setUser(result.data);
  };
  return (
